...
<?php
            ...
            $noinfo_div='<div class="d-flex justify-content-center pt-4" >  <p class="scoreCss"> No Winner is selected, all participant will be refunded automatically</p> </div>';
                $rowamount = mysqli_num_rows($sql_allContestQuery);
                if ($rowamount ==0){
                    echo $noinfo_div;
                }
        ...
        ?>
			<a class="linkColor" href="iframeAContestWinner.php?contestId=<?php echo $_GET['id'];?>&participantId=<?php echo $participantId;?>" >
			<div class='w-100 d-flex statusOfApprovalSelectContainer linkColor' style="overflow-x: hidden;">
			<div >
                <br>
                <span class="positionContentDiv"> <?php echo $xWinner; ?></span>
				<p style="font-weight:bold; display:block;"> Participant Name : <?php echo $participantName ?> </p> 
                <p style="font-weight:bold; display:block;"> Score : <?php echo $score ?></p>
			</div>
			</div>
			</a>
        <?php } ?>
