...
<body>
    ...
    <!-------------------------------------------------------- toggler -------------------------------------------------------->
        ...
        <div class="signUpMain">
            <div class="form">
            <div class="title">Organiser registration </div><br><a href="signUp_participant.php" style="width: 80%; font-size: 12px">sign up for participant</a><br><br>
            <form method="post">
                ...
                <label for="companyNo">Company number</label><br>
                <input type="number" placeholder="202005123456" name="companyNo" id="companyNo" required></input><br>
...
<?php
    if(isset($_POST['submit'])){
        ...
        $organiserNo_repeat = true;
        $sql_organiserNo = "SELECT companyNo FROM organiser;";
        $sql_organiserNoQuery = mysqli_query($con,$sql_organiserNo);
        while ($organiserNo = mysqli_fetch_array($sql_organiserNoQuery)){
            if ($organiserNo['companyNo'] == $_POST['companyNo']) {
                break;
            }
        }
        $organiserNo_repeat = false;
        ...
        if (($confirmPwValidation === true) && ($Emailvalidation === true) && ($organiserNo_repeat === false)) {
            $sqlAppend1 = "INSERT INTO login (loginEmail, loginPassword, userType) VALUES ('$_POST[email]', '$passw', 'organiser');";
            ...
            if(!mysqli_query($con, $sqlAppend1)){
                ...
                $sqlAppend2 = "INSERT INTO organiser (name, phoneNo, companyNo, street, city, state, postCode, country, loginId) 
                VALUES ('$_POST[name]','$_POST[phoneNo]', '$_POST[companyNo]', '$_POST[address]', '$_POST[city]', '$_POST[state]','$_POST[postcode]','$_POST[country]', '$userID[loginId]');";

                if(!mysqli_query($con, $sqlAppend2)){
                    die('error'.mysqli_error($con));

                }else {
                $_SESSION['userType'] = "organiser";
